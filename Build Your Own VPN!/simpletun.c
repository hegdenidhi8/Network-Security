...
#define TARGET_HOST "Nidhi"

#define TARGET_PORT "4433"
...
//  uint16_t total_len, ethertype;
  ...
  if (cliserv == CLIENT) {
    ...
    if (SSL_CTX_use_certificate_file(ctx, "/home/cs528user/client.crt", SSL_FILETYPE_PEM) <= 0) {
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx, "/home/cs528user/client.key", SSL_FILETYPE_PEM) <= 0) {
    ...
    if (SSL_CTX_load_verify_locations(ctx, "/home/cs528user/ca.crt", NULL) <= 0) {
    ...
    SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL); //Set to require peer (server) certificate verification

    SSL_CTX_set_verify_depth(ctx,1); //Set the verification depth to 1
    ...
    if (SSL_CTX_use_certificate_file(ctx, "/home/cs528user/server.crt", SSL_FILETYPE_PEM) <= 0) {
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx, "/home/cs528user/server.key", SSL_FILETYPE_PEM) <= 0) {
    ...
    SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);//Set to require peer (client) certificate verification

    SSL_CTX_set_verify_depth(ctx,1); //Set the verification depth to 1

    //X509_VERIFY_PARAM *vpm = SSL_CTX_get0_param(ctx);

    //X509_VERIFY_PARAM_set1_host(vpm, "Nidhi", 0);
  ...
  BONUS Code reffered from https://github.com/reginbald/IK2206-project-ssl-vpn, this helped me achieve 3 added features to the miniVPN.
  ...
  */
  ...
  if (FD_ISSET(ssl_fd, &rd_set)) { // from ssl
    ...
    if(FD_ISSET(tap_fd, &rd_set)){
      ...
      nwrite = sendto(net_fd, temp, nread + 32, 0, (struct sockaddr*)&remote, sizeof(remote)); 
    ...
    if(FD_ISSET(net_fd, &rd_set)){     
      ...
      /*nread = recvfrom(net_fd, (char *)&plength, sizeof(plength), 0,NULL, NULL);
