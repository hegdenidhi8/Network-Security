...
int main(int argc, char *argv[]){   
        ...
        if (buffer=calloc(DEFAULTFILESIZE,sizeof(char)))
        {
            ...
            if(!file_exists(fName))
            {
                ...
                if (!retVal)
                {
                    ...
                    memcpy(ctxtReceived,buffer,index-64); 
                    unsigned char macReceived[64];
                    memcpy(macReceived,buffer+(index-64),64); 
                    ...
                    {
                        getMAC(ctxtReceived,index-64,macGenerated,key,16); 
                        
                        if (memcmp(macReceived,macGenerated,64)!=0)
                        {
                            printf("MAC Authentication Failed!\n");
                            retVal=62;
                        }
                        ...
                        else
                        {
                            ...
                            decrypt(key,passphrase,ctxtReceived,p_ptxt,atoi(padASCII),index-64);
                            ...
                            if (f)
                            {
                                ...
                                {
                                    int pos=0;
                                    int contentLength=index-64-atoi(padASCII); 
                                    while(pos<contentLength) fputc(*(ptxt+pos++),f);
    ...
    if ((argc==3) && strncmp(*(argv+2),"-l",2)==0) //for decryption in 'local' mode
    {
        ...
        char fNameNew[strlen(*(argv+1))-3]; 
        strncpy(fNameNew,*(argv+1),strlen(*(argv+1))-3);
        fNameNew[strlen(*(argv+1))-4] = '\0';
